...
{
	nOffset = (nOffset) ? nOffset : 16;
	hashTable.nOffset = nOffset;
	hashTable.pHashTable = NULL;

	hashTable.pHashTable = malloc(HashSize(nOffset) * sizeof(void *));
	if (!hashTable.pHashTable)
	{
		fprintf(stderr, "%s\n", "HashTable初始化失败");
		exit(1);
	}
...
{
	uint32_t hashKey = HashKey(pKey, nSize, 0);
	haskKey = hashKey & HashMask(hashTable.nOffset);
	...
	while (pHead)
	{
		if (strncmp(pHead->pKey, pKey, nSize) == 0)
		{
			return pHead;
		}
		pHead = pHead->pNext;
